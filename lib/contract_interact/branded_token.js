...
const reqPrefix = '../..'
  ...
  , cache           = require(reqPrefix + '/lib/cache')  
  , stPrime         = require(reqPrefix + '/lib/contract_interact/st_prime')
;

//Variables
var contractName      = 'brandedToken'
...
var btAbi = JSON.parse(BrandedTokenContract.abi);
currContract = new web3RpcProvider.eth.Contract( btAbi );
    ...
    return Promise.all([senderBalancePromise, recipientBalancePromise])
      .then( responses => {
        ...
        const fundAndTransferPromise = oThis._fundSenderForTransferIfNeeded( transferParams )
          ...
          .catch( err => {
            console.log("=====Transaction Failed Catch :: Rollback Transfer =====");
  ...
  _transferInChain: function ( transferParams ) {
    ...
    return helper.call(web3RpcProvider, btAddress, encodeABI, {from: senderAddr}, transactionOutputs)
      .then(_ => {
        ...
        return helper.sendTxAsyncFromAddr(web3RpcProvider, btAddress, encodeABI, senderAddr, senderPassphrase, {from: senderAddr, gasPrice: coreConstants.OST_DEFAULT_GAS_PRICE})
