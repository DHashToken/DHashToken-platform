...
ERC20.prototype.transfer = function(sender, to, value) {
  ...
  return this.balanceOf(sender).then(balance => {
    ...
    Geth.UtilityChain.eth.personal.unlockAccount(sender).then(_ => {
      ...
      return this._erc20instance.methods.transfer(to, value).send({
        from: sender,
        gasPrice: coreConstants.OST_DEFAULT_GAS_PRICE
          ...
          Assert.equalsIgnoreCase(result.events.Transfer.returnValues._from, sender);
          Assert.equalsIgnoreCase(result.events.Transfer.returnValues._to, to);
...
ERC20.prototype.transferFrom = function(sender, from, to, value) {
  ...
  return this.balanceOf(from).then(balance => {
    ...
    Geth.UtilityChain.eth.personal.unlockAccount(sender).then(_ => {
      return this._erc20instance.methods.transferFrom(from, to, value).send({
        from: sender,
        gasPrice: coreConstants.OST_DEFAULT_GAS_PRICE
      }).then(result => {
...
ERC20.prototype.approve = function(sender, spender, value) {
  ...
  Geth.UtilityChain.eth.personal.unlockAccount(sender).then(_ => {
    return this._erc20instance.methods.approve(spender, value).send({
      from: sender,
      gasPrice: coreConstants.OST_DEFAULT_GAS_PRICE
    }).then(result => {
